...
with st.expander("About this app", expanded=False):
    st.markdown(
        ...
        "- Shows per-target metrics, OOB R², aggregated & per-target permutation importance, and diagnostics.\n"
...
expanded_names = get_transformed_feature_names(pipe.named_steps["prep"], num_cols, cat_cols)

imp_mean = result.importances_mean
imp_std = result.importances_std
if len(expanded_names) != len(imp_mean):
    st.warning("Feature name expansion mismatch with permutation importance results.")
    expanded_names = [f"f{i}" for i in range(len(imp_mean))]
...
def base_feature(name):
    # For one-hot names like 'Sour Region_Sour' → 'Sour Region'
    for c in cat_cols:
